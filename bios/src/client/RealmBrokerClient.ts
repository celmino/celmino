...
export namespace RealmBrokerClient {
    ...
    export const SetTenantAndGetSessionInfo = async (tenant_id: string): Promise<any> => {
        return new Promise((resolve, reject) => {
            const form = new FormData();
            form.append('tenant_id', tenant_id);

            HttpClient.Post(ConfigService.GetRealmOceanBrokerUrl() + 'SetTenantAndGetSessionInfo', form)
                .then(response => {
                    resolve(response.data);
                })
                .catch(error => {
