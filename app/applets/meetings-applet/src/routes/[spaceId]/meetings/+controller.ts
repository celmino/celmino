...
import { Button, ForEach, Fragment, HStack, Icons, MeetingProtocol, MenuButton, Segmented, Spacer, Spinner, UIController, UIRouteOutlet, UIView, VStack, cHorizontal, cLeading, cTop, getAppFullName, useNavigate, useParams, useProtocol } from "@tuval/forms";
                        ...
                        isLoading ? Spinner() :
                            VStack({ spacing: 10 })(
                                ...ForEach(meetings)(meeting =>
                                    HStack(
                                        HStack({ alignment: cLeading })(
                                            meeting.is_template === 0 ?
                                                HStack(
                                                    Text(moment(meeting.start_at).date())
                                                        // .foregroundColor('#5F6B7A')
                                                        .fontFamily('Poppins, sans-serif, Helvetica, Arial')
                                                        .fontSize(19)
                                                        .fontWeight('700')
                                                ).width(35).height(35).background('#0072CE').cornerRadius('50%').foregroundColor('white') : Fragment()
                                            ...
                                            Text(meeting.name).fontSize(13).fontWeight('700')
                                                .padding(cHorizontal, 10)
